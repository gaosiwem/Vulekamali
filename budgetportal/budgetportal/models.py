...
class FinancialYear(models.Model):
    ...
    def get_closest_match(self, department):
        sphere = self.spheres.filter(
            slug=department.government.sphere.slug).first()
        government = sphere.governments.filter(
            slug=department.government.slug).first()
        department = government.departments.filter(
            slug=department.slug).first()
        if not department:
...
class Sphere(models.Model):
    ...
    class Meta:
        unique_together = (("financial_year", "slug"),
                           ("financial_year", "name"))
...
class ShowcaseItem(SortableMixin):
    ...
    cta_text_1 = models.CharField(
        max_length=200, verbose_name="Call to action text 1")
...
class Department(models.Model):
    ...
    #     if (
    #         self._estimates_of_econ_classes_expenditure_dataset.get(
    #             level, None)
    #         is not None
    #     ):
    ...
    #     cuts = []
    #     if self.government.sphere.slug == PROVINCIAL_SLUG:
    #         cuts.append(openspending_api.get_geo_ref() + ':"%s"' %
    ...
    #                 }
    #             econ_classes[cell[econ_class_2_ref]
    #                          ]["items"].append(new_econ_2_object)
    #     # sort by name
    #     def name_func(x): return x["name"]
    ...
    #         raise Exception(
    #             "Could not calculate total change for department budget")

    #     return total_voted, total_adjusted

    # def _get_budget_virements(self, openspending_api, dataset, total_voted):
    #     virements_resource = dataset.get_resource(
    #         "CSV", name="Value of Virements")
    ...
    #     dataset = get_expenditure_time_series_dataset(
    #         self.government.sphere.slug)
    #     if not dataset:
    #         return None
    #     openspending_api = dataset.get_openspending_api()
    ...
    #     except KeyError:
    #         raise Exception(
    #             "An invalid phase was provided: {}".format(budget_phase))
    ...
    #                 notice_string += " {}".format(
    #                     " and ".join(no_data_for_years))
    #             else:
    #                 notice_string += " {}".format(
    #                     ", ".join(no_data_for_years[:-1]))
    ...
    # def get_expenditure_time_series_by_programme(self):
        financial_year_start = self.get_financial_year().get_starting_year()
        financial_year_start_int = int(financial_year_start)
        year_ints = range(financial_year_start_int - 3,
                          financial_year_start_int + 1)
                ...
                for fiscal_phase in EXPENDITURE_TIME_SERIES_PHASES:
                    for program in programmes:
                        ...
                        if not found:
                            ...
                            if fiscal_year not in missing_phases_count:
                                missing_phases_count[fiscal_year] = {
                                    program: 1}
                            else:
                                if (
                                    program
...
class InfraProjectSnapshot(models.Model):
    ...
    local_municipality = models.CharField(
        max_length=1024, blank=True, null=True)
    district_municipality = models.CharField(
        max_length=1024, blank=True, null=True)
    ...
    primary_funding_source = models.CharField(
        max_length=1024, blank=True, null=True)
    nature_of_investment = models.CharField(
        max_length=1024, blank=True, null=True)
